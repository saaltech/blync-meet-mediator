...
RUN mkdir /saal-web-repo
COPY --from=dpkg-package /saal-repo/jitsi-meet-web-config_*.deb /saal-web-repo/
COPY --from=dpkg-package /saal-repo/jitsi-meet-web_*.deb /saal-web-repo/

RUN \
  apt-get update && \
	apt-get install -y cron nginx-extras && \
	apt-get -d install -y /saal-web-repo/jitsi-meet-web-config_*.deb && \
	apt-get install -y /saal-web-repo/jitsi-meet-web_*.deb && \
  dpkg -x /saal-web-repo/jitsi-meet-web-config_*.deb /jitsi-meet-web-config && \
  dpkg -x /saal-web-repo/jitsi-meet-web_*.deb /jitsi-meet-web && \
  ...
  rm -rf /var/lib/apt/lists/ && \
	...
	rm -rf /jitsi-meet-web-config /jitsi-meet-web /var/cache/apt
